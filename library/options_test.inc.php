...
//Required for sub-item display
require_once(dirname(__FILE__) ."/classes/display/DataDisplayObjectFactory.php");
require_once(dirname(__FILE__) ."/classes/display/SubItem.php");
require_once(dirname(__FILE__) ."/classes/display/TableHelper.php");
...
function generate_form_field($frow, $currvalue) {
  ...
  //SubItems
	...
	//GroupName[Row][Item]
  	...
  	//"form_".$frow['group_name']."[1][".$frow['field_id']."]"
  	//Draw Container
  	//Should really return the value.
  	echo "<td class='text data' colspan='1' rowspan='3'";
  	echo "><div style=\"overflow : auto; height: 240px; width: 500px; background: red;\">";
  	//Run through sub items.
  	 
  	$subItem = new SubItem($frow['field_id']);
  	$subItem->getDatafromDatabase();
  	if($frow['field_id'] == "insurance_company") { 
  		$subItemData = DataDisplayObjectFactory::build("BusinessFlatTable","insurance_company");
  	}
  	else {
  		$subItemData = DataDisplayObjectFactory::build($subItem->getTableName()."Table");
  	}
  	
  	// Need to update DataDisplayObjectFactory. http://stackoverflow.com/questions/3964793/php-case-insensitive-version-of-file-exists
  	$subItemData->setSourceIdValue($currvalue[0]);
  	
  	$firstLine = true;
  	echo "<table class=\"tableRepeat\"> <thead><tr>";
  	foreach($subItem->_subItemRows as $row) {
  		echo "<td>".$row->title ."</td>\n";
  	}
  	echo "</tr></thead>";
  	$rowCount = 1;
  	while($rowValue = $subItemData->getData()) {
  		echo "<tr>";
  		
  		
  		//Generate Hidden Value for id from database
  		generate_form_field(
  		array("data_type" => 37, "field_id" => "sub_".$frow['field_id']."[".$rowCount."][db_id]", "list_id" => $row->list_id,
  		"description" => $row->description, "fld_length" => $row->fld_length,
  		"max_length" => $row->max_length, "edit_options" => $row->edit_options),
  		$rowValue['id']);
  		
  		generate_form_field(
  		array("data_type" => 37, "field_id" => "sub_".$frow['field_id']."[".$rowCount."][db_action]", "list_id" => $row->list_id,
  		"description" => $row->description, "fld_length" => $row->fld_length,
  		"max_length" => $row->max_length, "edit_options" => $row->edit_options),"update");
  		...
  		foreach($subItem->_subItemRows as $row) {
  			...
  			if($row->dataType == 36) {
  				$field_id = $row->tableName; 
  			}
  			else {
  			  	$field_id = "sub_".$frow['field_id']."[".$rowCount."][".$row->tableName.".".$row->columnName ."]";
  			}
  			
  			
  			generate_form_field(
  					array("data_type" => $row->dataType, "field_id" => $field_id, "list_id" => $row->list_id,
  							"description" => $row->description, "fld_length" => $row->fld_length,
  							"max_length" => $row->max_length, "edit_options" => $row->edit_options),
  	...
  	echo "</tr>";
  	echo "</table>";
  	echo "<button id=\"addRow\">Add Row</button>";
...
function generate_display_field($frow, $currvalue) {
  ...
  // Sub-Item
  	...
  	//Should really return the value.
  	echo "<td class='text data' colspan='1' rowspan='3'";
  	echo "><div style=\"overflow : auto; height: 80px; width: 230px; background: red;\">";
  	//Run through sub items.
	  	...
	  	while($rowValue = $subItemData->getData()) {
	  		...
	  		foreach($subItem->_subItemRows as $row) {
	  			...
	  			echo generate_display_field(
		  			array("data_type" => $row->dataType, "field_id" => $row->tableName.".".$row->columnName, "list_id" => 17), 
		  			$rowValue[$row->columnName]);
	  			echo "</td>";
	  			// escaped variables to use in html
  ...
  //Action
  	...
  	//http://localhost/openemr-demographics/interface/super/show_lists_popup.php
  	//interface/super/form_popup.php?type={actionName}&sourceId={0}&destVal={1}
  	//Need to add openEMR source
  	$s = "<a href=\"".$GLOBALS['webroot']."/interface/super/forms_popup.php?type=insurance&sourceId=23\">Edit</a>";
...
//Main function being used for changes.
function display_layout_tabs_data_ext($formtype, $displayType = "display", $result1, $result2='') {
	global $item_count, $cell_count, $last_group, $CPR;

	//<?php $subItem = new SubItem("person_address");
	// $subItem->getDatafromDatabase();
	// $subItemData = DataDisplayObjectFactory::build($subItem->getTableName()."Table");
	// // Need to update DataDisplayObjectFactory. http://stackoverflow.com/questions/3964793/php-case-insensitive-version-of-file-exists
	// $subItemData->setSourceIdValue(23);
	// echo "<br/>Here comes the data<br/><br/>";
	// echo "<div style=\"overflow : auto; height:80px\">";
	// $subItem->display($subItemData);
	
	//Get all groups for a type.
	
	$isEditable = strcmp($displayType, "display") ==0 ? false : true;
	...
	while ($frow = sqlFetchArray($fres)) {
		...
		<div class="tab <?php echo $first ? 'current' : '' ?>">
			...
			<?php
				//Displaying sub-tabs would go here.
				if($this_group === "7InsuranceTest") {
					echo "Tabs will go here";
					$fres1 = sqlStatement("select patient.patient_id from patient ".
						" inner join person on patient.person_id = person.person_id ".
						" where person.person_id = ?", array(23));
					$frow1 = sqlFetchArray($fres1);
					$patient_id = $frow1['patient_id'];
				}	
				$table = new TableHelper($CPR);
				$x =0;
				$y =0;
				...
				while ($group_fields = sqlFetchArray($group_fields_query)) {
					...
					$height = isset($group_fields['height']) ? ($group_fields['height']) : 1;
					
					//Loop though any block already done.
					while($table->hasDrawn($x, $y) || $x > ($CPR -1)) {
						if($x > ($CPR-1)) {
							disp_end_row();
							echo "<tr>";
							$x =0;
					...
					// Handle starting of a new row.
					if (($titlecols > 0 && $cell_count >= $CPR) || $cell_count == 0) {
					  disp_end_row();  //Not sure if this should be here.
					  $table->nextRow();
					  $x =0;
					  $y++;
					  echo "<tr>";
					...
					// Handle starting of a new label cell.
						if ($titlecols > 0) {
						  ...
						  echo "<td class='label' colspan='$titlecols_esc' ";
						  echo ">";
						  $table->addItem($titlecols_esc,1);
						  $table->drawBox($titlecols_esc,1);
						  $cell_count += $titlecols;
						}
						++$item_count;
						...
						if ($datacols > 0) {
						  ...
						  echo ">";
						  $cell_count += $datacols;
						  $table->addItem($datacols,1);
						  $table->drawBox($datacols,1);
						}
						//Moved since we have to have an inner DIV
						++$item_count;
						if($isEditable) {
						...
						//Need to get patient value
						if($isEditable) {
							echo generate_form_field($group_fields, array(23,$field_id));
						}
						else {
							echo generate_display_field($group_fields, array(23,$field_id));
						}
						$table->addItem(1,3);
						$table->drawBox($datacols,1);
...
function display_layout_tabs_data_editable_ext($formtype, $result1, $result2='') {
	global $item_count, $cell_count, $last_group, $CPR;
