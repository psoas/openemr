...
if(isset($_GET['mode']) && strcasecmp($_GET['mode'],"subitem") == 0) {
	$fetchList = false;
	$sqlStatement = "select distinct group_name as `option_id`,group_name as `title`  from layout_options
 where form_id = 'SUB'";
} 
else { //Must be normal mode.
	$fetchList = true;
	$sqlStatement = "SELECT * FROM list_options WHERE " .
		"list_id = 'lists' ORDER BY title";
}
$res = sqlStatement($sqlStatement);
